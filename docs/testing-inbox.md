...
✅ Behat 9/9 — erledigt (commit `b8b2f2b`)
✅ Grunt AMD rebuild — erledigt (commit `052255d`)
✅ PHPCS 0 errors / 0 warnings — erledigt (commit `0ec51f5`)

---

### 1. GitHub Actions CI — muss grün sein ← nächster Schritt

Laufender Check: https://github.com/jmoskaliuk/mod_elediacheckin/actions

Alle Jobs müssen grün sein: phplint, phpcs, phpdoc, savepoints, mustache,
grunt, phpunit, behat.

---

### 2. Deinstallations-Test (nie getestet — Pflicht für Plugin Directory)
...
### 3. DEBUG_DEVELOPER — keine PHP-Warnungen/Notices

Moodle muss mit `$CFG->debug = DEBUG_DEVELOPER` und `$CFG->debugdisplay = 1`
laufen (ist auf der Demo-Instanz i.d.R. schon aktiv). Sicherstellen:

1. Aktivität öffnen, Frage beantworten, Einstellungen öffnen — keine
   gelbe Warnbox erscheint.
2. Admin: Seite neu installieren oder Upgrade erzwingen — keine Notices in
   der Upgrade-Ausgabe.

Schnellcheck:
```
docker compose -f ~/demo/compose.yml exec -T webserver \
  php -r "define('CLI_SCRIPT',true); require '/var/www/site/moodle/public/config.php'; echo \$CFG->debug . PHP_EOL;"
```
Ausgabe muss `32767` (= DEBUG_DEVELOPER) sein.

---

### 4. Non-default DB-Prefix-Test

Plugin Directory QA testet mit Prefix `mqa_` statt `mdl_`. Falls irgendwo
`mdl_` hardcoded ist, schlägt die Installation fehl.

```
docker compose -f ~/demo/compose.yml exec -T webserver \
  grep -r "mdl_" /var/www/site/moodle/public/mod/elediacheckin/ \
    --include="*.php" --include="*.xml" | grep -v "\.git"
```

Erwartetes Ergebnis: keine Ausgabe (= kein hardcoded Prefix).
...
### 7. Repository umbenennen für Plugin Directory

Plugin Directory erwartet Repo-Namen `moodle-mod_elediacheckin`, nicht `mod_elediacheckin`.
Das ist kein technisches Blocker (Plugin läuft trotzdem), aber Reviewer
werden es bemängeln.

Aktion: GitHub Repo umbenennen unter
https://github.com/jmoskaliuk/mod_elediacheckin/settings
→ Repository name → `moodle-mod_elediacheckin`

(GitHub leitet alte URLs automatisch weiter — kein Push/Clone nötig.)
